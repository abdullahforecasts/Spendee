...
class _FriendsPageState extends State<FriendsPage> {
  ...
  void _filterFriends(String query) {
    ...
    setState(() {
      friends = allFriends.where((f) => (f['name'] as String).toLowerCase().contains(q)).toList();
    });
  }
  ...
  void _removeFriendAt(int index) {
    setState(() {
      ...
      allFriends.removeWhere((f) => f['name'] == removed['name']);
    });
  }
  ...
  void _confirmRemoveFriend(String name, int index) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text("Remove $name?", style: GoogleFonts.poppins(fontWeight: FontWeight.w600)),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        style: GoogleFonts.poppins(fontWeight: FontWeight.w500, fontSize: 15 * scale),
                        decoration: InputDecoration(
                          hintText: "Search friend...",
                          hintStyle: GoogleFonts.poppins(color: Colors.grey.shade600, fontSize: 14 * scale),
                          prefixIcon: const Icon(Icons.search),
                          filled: true,
                          fillColor: Colors.white,
                          contentPadding: EdgeInsets.symmetric(vertical: 16 * scale, horizontal: 16 * scale),
                          border: OutlineInputBorder(borderRadius: BorderRadius.circular(30 * scale), borderSide: BorderSide.none),
                        ),
                      ...
                      if (friends.isEmpty)
                        Padding(
                          ...
                          child: Text("No friends found", style: GoogleFonts.poppins(color: Colors.grey)),
                        ),
              ...
              child: Center(
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text("Add", style: GoogleFonts.poppins(fontSize: 16 * scale, fontWeight: FontWeight.w600, color: Colors.white)),
  ...
  Widget _buildFriendTile(Map<String, dynamic> friend, int index, double scale) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 5, offset: Offset(0, 3 * scale))],
      ),
      child: ListTile(
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const OthersProfileViewPage())),
        ...
        title: Text(
          friend['name'],
          style: GoogleFonts.poppins(fontWeight: FontWeight.w600, color: Colors.white, fontSize: 14 * scale),
        ),
        trailing: PopupMenuButton<String>(
          icon: Icon(Icons.more_vert, color: Colors.white, size: 20 * scale),
          onSelected: (value) {
            if (value == 'remove') _confirmRemoveFriend(friend['name'], index);
          },
          itemBuilder: (context) => [const PopupMenuItem(value: 'remove', child: Text('Remove from list?'))],
        ),
      ),
    );
  }
